...
void fight(Stat *p1, Stat *p2, char ch) {
    ...
    damage = random1(p1->defense + x) - random1(p2->attack + y);
    if (damage > 0) {
        p1->health -= damage;
...
void initializePositions(GameState *game) {
    game->playerX = random1(9);
    game->playerY = random1(9);

    for (int i = 0; i < 7; i++) {
        game->banditX[i] = random1(9);
        game->banditY[i] = random1(9);
    }
    
    for (int i = 0; i < 3; i++) {
        game->itemX[i] = random1(9);
        game->itemY[i] = random1(9);
    }
    
    for (int i = 0; i < 2; i++) {
        game->powerUpX[i] = random1(9);
        game->powerUpY[i] = random1(9);
    }
    
    for (int i = 0; i < 3; i++) {
        game->trapX[i] = random1(9);
        game->trapY[i] = random1(9);
    }
    return ;
}
...
int playStealth() {
    ...
    printf("\nFew days later a small village you encouter has been robbed.And you can gain so much if you help them.\n");
        ...
        if(caught>0 && game.hasPowerUp > 0){
           ...
           caught=0; 
            ...
            if (success) {
                ...
                do {
                    game.playerX = random1(10 - 1);
                    game.playerY = random1(10 - 1);
...
int main(){
  printBold("Welcome to \"Voyager's Fate\" .\n\n");
    printBold("Introduction:\n");
    printf("Embark on \"Voyager's Fate\", an adventure which determines do you really have the luck the world so furiously wants.\n");
    printBold("The best ending is calculated to be approximately 0.00582% where you also win a total luck game with 100% probability and if you get the best win with 50% probability.\n");
    printBold("\n\nAbout the Game\n");
